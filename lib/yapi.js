				...
				if(err) {
					...
					function loopRead(obj) {
						if(!obj.list) {
							...
							let str = ApiFuncTemplate.replace('{{funcName}}', funcName).replace('{{url}}', url).replace('{{method}}', obj.method).replace('{{params}}', params).replace('title', obj.title).replace('{{dataKey}}', dataKey)
